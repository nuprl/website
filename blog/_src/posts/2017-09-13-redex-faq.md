...
If a term matches the pattern, then Redex can show how the _named non-terminals_
 in the pattern bind to sub-terms.
...
- The **...** pattern matches any sequence of the preceding pattern.
  If a named non-terminal appears before a **...**, then the non-terminal
  binds a sequence of subterms.
...
- Non-regular patterns. For example, one non-regular pattern is "all lists
  that contain some number of integers followed by the same number of strings".
 ...
 **(where pattern term)**, it checks that each _pattern_ matches the term.
If not, Redex stops applying this rule.
...
For example, a type inference judgment may take an input term and output a type.
...
Two things to note about the reduction relation **step**:
...
2. Using **in-hole** on the second argument of **-->** puts a new value
   back into the "right place" in the evaluation context.
3. The unquote operator (**,**) escapes to "Racket mode" to evaluate
...
Expressions in a Redex model are sometimes evaluated in "Racket mode" and
...
4. ... and **term**s inside an unquoted sub-expression are evalatued in Redex mode.
...
Redex will raise an exception when it sees an unquote in Racket mode.
...
- Contribute to this FAQ: <https://github.com/nuprl/website/>
